...
label.pack(side=TOP)
filename = PhotoImage(file = r"C:/Users/samya/OneDrive/Desktop/dd1/dd1/demo.png")
...
subm2.add_command(label="Driver",command=anotherWin)
...
def webrec():
   ...
   op=cv2.VideoWriter('Sample1.avi',fourcc,1.0,(640,480))
   while True:
      ret,frame=capture.read()
      gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
      cv2.imshow('frame',frame)
...
def alert():
   mixer.init()
   alert=mixer.Sound(r'C:/Users/samya/OneDrive/Desktop/dd1/dd1/beep-07.wav')
   alert.play()
   time.sleep(5)
   alert.play()   
...
but5=Button(frame,padx=5,pady=5,width=39,bg='white',fg='black',relief=GROOVE,command=blink,text='Detect Eye Blink With Sound',font=('helvetica 15 bold'))
